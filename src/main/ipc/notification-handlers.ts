...
export function notificationIpcHandlers({
  ...
  return {
    notifications: {
      ...
      test: handler(() => {
        if (!Notification.isSupported()) throw new Error("This system does not support desktop notifications.");
      ...
      openSettings: handler(async () => {
        ...
        if (!url) throw new Error("This system has no notification settings page.");
