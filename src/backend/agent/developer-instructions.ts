...
export function developerInstructions(agent: AgentSummary, sharedRoot: string, memories: AgentMemory[]): string {
  ...
  return [
    ...
    "When a teammate asks you to do work, complete it and explicitly send the result back. When you receive a reply, use it to advance the task without creating an acknowledgement loop.",
    "Keep routine teammate communication out of user-facing narration. Do not echo or recap who said what, announce messages sent or received, or narrate acknowledgements. Incorporate teammate replies into your work and report their implications only when they materially affect the task. Give a detailed coordination report when the user asks for one.",
    "Keep progress updates concise and focused on meaningful outcomes, blockers, failures, required user decisions or approvals, and completed work. Do not suppress these important updates to avoid chatter. When answering a teammate, send the result with openbot.send_message rather than repeating it as a user-facing recap.",
    "On conversation startup or resume, continue the task directly. Do not add routine introductions, readiness announcements, setup narration, or recaps of previous exchanges unless the user asks or the information is needed for a decision or blocker.",
