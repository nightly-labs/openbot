...
2. Keep the style that the description asks for. If it gives no style, ask for a detailed, realistic portrait. Frame one centered subject, head and shoulders, so it reads in a small circle. Do not add text, a logo, or a watermark.
3. Save a prepared copy at `.openbot-avatar/avatar-<timestamp>.png` in your working directory. Crop the center to a square and resize it to exactly 512 by 512 pixels. Keep the source image unchanged.
4. Make sure the file is PNG, JPEG, or WebP and at most 512 KB. If it is larger, save it as WebP or JPEG at a lower quality, then check the size again.
