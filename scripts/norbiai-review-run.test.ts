...
// Prints the review, and an `exec` block first when the reviewer reads with a tool.
...
type Run = { review: string; reads: boolean; inlineLimit?: number };

/** Runs the workflow step as the runner would, and reads back its outputs and prompts. */
function review({ review, reads, inlineLimit }: Run) {
  ...
  for (const file of ["title", "body", "previous", "withdrawn", "responses", "output"]) {
...
describe("NorbiAI review run", () => {
  ...
  it("puts the files that fit in the prompt and lists the rest to read", () => {
    const run = review({ review: CLEAN + LEDGER, reads: true, inlineLimit: 2_000 });
  ...
  it("retries once and then fails a review that read none of the diff left out of the prompt", () => {
    const run = review({ review: CLEAN + LEDGER, reads: false, inlineLimit: 2_000 });
