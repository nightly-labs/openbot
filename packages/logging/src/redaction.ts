...
// A home directory names the operating-system account, and a provider error
// message very often *is* a path - `~/.local/bin/codex` against
// `/opt/homebrew/bin/codex` is the distinction a diagnostic needs, and the
// user name in front of it is not. Abbreviating to `~` keeps the first and
// drops the second. The name run is bounded like the rules above, and stops
// at the separator so the rest of the path survives.
const HOME_PATH = /(?:\/Users\/|\/home\/|[A-Za-z]:\\Users\\)[A-Za-z0-9._-]{1,64}/gu;
...
// How much redacted text may leave the machine on an analytics event, against
// `MAX_PARAM_LENGTH` for a line that stays on it. Two hundred characters is
// enough to tell two failures apart and short enough that a message cannot
// carry a payload.
export const MAX_ANALYTICS_MESSAGE_LENGTH = 200;

// The one shape free text takes when it is sent rather than written: redacted,
// collapsed onto a single line, and cut. Callers never truncate themselves -
// the sanitizers call this, so no call site can send more by forgetting to.
export function redactedSummary(value: string): string {
  const redacted = redactText(value).replace(/\s+/gu, " ").trim();
  return redacted.length > MAX_ANALYTICS_MESSAGE_LENGTH
    ? `${redacted.slice(0, MAX_ANALYTICS_MESSAGE_LENGTH - 1)}…`
    : redacted;
}
