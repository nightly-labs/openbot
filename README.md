...
| `bun run lint` | Check the whole repository with two Biome worker threads. Set `RAYON_NUM_THREADS` to change the limit. |
| `bun run typecheck` | Check all 11 projects in sequence with cached results and two compiler threads. Set `GOMAXPROCS` to change the limit. |
