...
| `bun run types:ratchet` | Hold each TypeScript option in `tools/typescript/type-baseline.json` to its error count per file in the Node and web projects. A new error fails. `--write` and `--add=<option>` work as in `lint:ratchet`. |
