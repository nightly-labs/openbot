...
- `openbot-approval-automation-v1.json` holds Turbo mode and the agents granted "Always allow". It
  ...
  remote host's behalf. `AttentionRegistry` reads it at each approval and at each question the agent
  asks, which a grant skips so the agent decides for itself. A hosted-site mutation and a question
  that asks for a secret stay outside any grant it can carry.
